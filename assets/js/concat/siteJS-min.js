...
var showNowDay = function showNowDay() {
...
function getLastReport() {
  ...
  $.get(valStore.path + 'getLastReport.php?name=' + valStore.userName, function (data) {
    console.log(data);
...
function genDeleteList() {
  ...
  $.get(paths.reportEditor + 'removeRecord&object=' + record.parsetoJSON(), function (data) {}).done(function () {
    toggleSelectElemetnTable();
...
function tableSortAlg() {
  ...
  $('#tableReport th').on('click', function () {
    ...
    if ($(this).html()) {
      ...
      if ($(this).attr('class')) $(this).attr('sortNum', index);else $(this).attr('sortNum', '');
      ...
      $('#tableReport tbody tr').disassembleTable(['name', 'userName', 'account', 'descr', 'jobType', 'dateJob', 'hoursJob']).keySort(sorto).renderSortReport('#tableReport tbody').querySelectorAll("tr").highlightRowTable('td.' + (typeof nameList[0] == 'undefined' ? 'dateJob' : nameList[0]));
      // console.clear()
    }
  });
...
Object.prototype.highlightRowTable = function (colName) {
  ...
  $(this).find(colName).each(function (index, element) {
    if (colName == 'name') allVal.push($(this).children('span').text() + $(this).html().split('<br>')[0]);else allVal.push($(this).html());
  ...
  $.each(result, function (index, value) {
    ...
    if (colName == 'td.dateJob') {
      var sumChar = "position: absolute;text-shadow:0px 1px 1px white;color:grey;font-size:14px;margin-top:30px;margin-left:10px;content: '" + summH[index] + " h';font-family: FontAwesome;font-style: normal;font-weight: bold;text-decoration: inherit;";
      $('<style>#' + value + ':after{' + sumChar + '}</style>').appendTo('head');
    }
  });

  var alltimeChar = alltime > 40 ? alltime + ' h (OverTime: ' + (alltime - 40) + ' h)' : alltime + " h",
...
var print = function print(obj, delp, delo, ind) {
...
function getDateForDB(dateOfWeek, func) {
  ...
  $('#table-c td').on('click', function () {
    if ($(this).html() != '-') {
      ...
      var date = new Date($('#calendar-box').attr('data-year') + "-" + todayMonthFromCalend() + "-" + $(this).html()),
...
function changeButton(switcher, buttonID) {
  if (switcher == 'disable') $(buttonID).addClass('disableButton');else $(buttonID).removeClass('disableButton');
}
...
Object.prototype.watchFromJobType = function (checkValue, specRun) {
  ...
  var switchSlectBlock = function switchSlectBlock(o) {
    ...
    for (var i = 0; i < checkValue.length; i++) {
      ...
      firstCorrectingRun = function firstCorrectingRun(obj, swt) {
    if (swt) {
      var _this = obj;
...
insertNewReport.prototype = {
  ...
  getValues: function getValues() {
    ...
    $('#table-c td.today').each(function (i) {
      dateEdit[i] = momentShort(new Date($(this).text() != '' ? $('#calendar-box').attr('data-year') + " " + todayMonthFromCalend($(this)) + " " + $(this).html() : new Date()));
    });
  ...
  pushData: function pushData() {
    ...
    var startOfWeek = momentShort(this.dateEdit[0], 1),
...
var initAddAlg = function initAddAlg() {
  var insertNew = insertNew || new insertNewReport();
  insertNew.install().valid().getValues().pushData().closeEvent();
},
    showSQLError = function showSQLError(data) {
  if (data) {
